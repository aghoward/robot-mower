...
PROJECT ?=$(shell ls *.ino | sed 's/\.ino//g')
...
ifdef MIGHTY1284P
ARDUINO_MODEL ?=avr_developers
AVRDUDE_PROGRAMMER ?=arduino
CUSTOM_ARDUINO_DIR=/home/howard/Arduino
ARDUINO_VARIANT ?=$(CUSTOM_ARDUINO_DIR)/hardware/mighty1284p/avr/variants/avr_developers
ARDUINO_BOARDS_FILE ?=$(CUSTOM_ARDUINO_DIR)/hardware/mighty1284p/avr/boards.txt
endif
#ARDUINO_MODEL = uno
...
#ARDUINO_FAMILY = mega
#ARDUINO_FAMILY =atmega328

# Arduino variant (for Arduino 1.0+).
# Directory containing the pins_arduino.h file.
ARDUINO_VARIANT ?=$(ARDUINO_DIR)/hardware/arduino/avr/variants/$(VARIANT)
...
PORT ?=	$(shell ls /dev/tty* | grep "ACM\|USB")
...
#ARDUINO_LIBS += EEPROM
#ARDUINO_LIBS += HID
#ARDUINO_LIBS += NRFLite
ARDUINO_LIBS += Servo
#ARDUINO_LIBS += SoftwareSerial
ARDUINO_LIBS += SPI
ARDUINO_LIBS += arduino-timer
#ARDUINO_LIBS += Wire
...
# Give the name of the directory containing the library source files.
USER_LIBDIR ?=	/home/ahoward/Arduino/libraries
USER_LIBS ?=
...
ARDUINO_CORE ?= $(ARDUINO_DIR)/hardware/arduino/avr/cores/arduino
ARDUINO_BOARDS_FILE ?=$(ARDUINO_DIR)/hardware/arduino/avr/boards.txt
...
MODEL_PATTERN_MATCHING = $(ARDUINO_MODEL)\|$(ARDUINO_FAMILY)\|$(ARDUINO_FAMILY).menu.cpu.$(ARDUINO_MODEL)
else
...
ALIBDIRS = $(wildcard \
		...
		$(ARDUINO_LIBS:%=$(ARDUINO_DIR)/hardware/arduino/avr/libraries/%/src) \
		$(ARDUINO_LIBS:%=$(ARDUINO_DIR)/hardware/arduino/avr/libraries/%/src/bsp/source) \
		$(ARDUINO_LIBS:%=$(ARDUINO_DIR)/hardware/arduino/avr/libraries/%/src/driver/source) \
		$(ARDUINO_LIBS:%=$(ARDUINO_DIR)/hardware/arduino/avr/libraries/%/src/socket/source) \
		$(ARDUINO_LIBS:%=$(ARDUINO_DIR)/hardware/arduino/avr/libraries/%/src/bus_wrapper/source) \
		$(ARDUINO_LIBS:%=$(ARDUINO_DIR)/hardware/arduino/avr/libraries/%/src/common/source) \
		$(ARDUINO_LIBS:%=$(ARDUINO_DIR)/hardware/arduino/avr/libraries/%/src/spi_flash/source) \
		$(ARDUINO_LIBS:%=$(ARDUINO_DIR)/hardware/arduino/avr/libraries/%/src/$(ARCH)) \
		$(ARDUINO_LIBS:%=$(ARDUINO_DIR)/hardware/arduino/avr/libraries/%/src/utility) \
...
OPT_OTHER += -fno-use-cxa-atexit -fno-threadsafe-statics
endif
...
STRIP =		$(AVR_TOOLS_PATH)$(AVRPREFIX)strip
